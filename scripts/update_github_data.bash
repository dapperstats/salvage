...
git checkout master
git add data/*.csv
git commit -m "updating_data Travis commit: $TRAVIS_COMMIT"
...
curl -v -i -X POST -H "Content-Type:application/json" -H "Authorization: token $GITHUB_RELEASE_PAT" https://api.github.com/repos/dapperstats/salvage/releases -d "{\"tag_name\":\"$TRAVIS_COMMIT\"}"